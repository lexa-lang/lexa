...
let lookup_type_name (con_name : var) : var =
  ...
  match res with
  | Some (type_name, _) -> sprintf "__%s_t__" type_name
...
|} compute_args type_name type_name type_name con_name assign_args
    | Match {match_expr; pattern_matching} ->
      let gen_patter_matching (i : int) (pattern, clause_body) =
        (match pattern with
        | PTypecon (con_name, args) ->
          let type_name = lookup_type_name con_name in
          let bind_args_str = String.concat ""
            (List.mapi (fun i arg_name -> sprintf "i64 %s = (i64)(((%s*)__expr_res__)->%s[%d]);\n" arg_name type_name con_name i) args)
          in
          let clause_body_str = 
            sprintf "{%s\n%s;}"
              bind_args_str
          ...
          in
          if i = 0 then
            sprintf "if (((%s*)__expr_res__)->tag == %s) {__match_res__=(%s);}" type_name con_name clause_body_str
          else
            sprintf "else if (((%s*)__expr_res__)->tag == %s) {__match_res__=(%s);}" type_name con_name clause_body_str)
      in
      let pattern_matching_str = String.concat "\n" (List.mapi gen_patter_matching pattern_matching) in
      sprintf "({i64 __match_res__;i64 __expr_res__=(i64)%s;\n%s\n__match_res__;})" 
      (gen_expr match_expr) pattern_matching_str
  )
  in
  (match e with
...
let rec eff_type_pass (toplevel : top_level list) : eff_type_env =
  ...
  | (TLObj (obj_name, _, operation_list)) :: tail ->
      (List.map (fun {op_anno; op_name; _} -> ({obj_name; op_name}, op_anno)) operation_list)
      @ (eff_type_pass tail)
...
return((int)__res__);}|}
  ...
  | TLObj (obj_name, obj_params, hdls) -> 
    let gen_hdl {op_anno; op_name; op_params; op_body} = 
      let concated_params = (List.map (fun x -> (CTI64P, x)) obj_params) 
        @ (List.map (fun x -> (CTI64, x)) op_params) in
  ...
  | TLType typedefs ->
    ...
    let gen_struct ({type_name; type_cons} : typedef) =
      sprintf "typedef struct __%s_t__ {\nenum %s tag;\nunion {\n%s};\n} __%s_t__;\n" 
        type_name 
        (sprintf "__%s_tag__" type_name) 
        (gen_cons type_cons)
        type_name
