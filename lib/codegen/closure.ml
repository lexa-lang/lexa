...
let rec convert_expr (e : Syntax.expr) (env : var list) =
  match e with
  | Syntax.Var x -> Var x
  | Syntax.Int i -> Int i
  ...
  | Syntax.Let (x, e1, e2) -> Let (x, convert_expr e1 env, convert_expr e2 (x :: env))
  | Syntax.If (e1, e2, e3) -> If (convert_expr e1 env, convert_expr e2 env, convert_expr e3 env)
  | Syntax.Fun (params, body) ->
    let body' = convert_expr body (params @ env) in
  ...
  | Syntax.App (e, es) -> 
    (match e with
    | Var name -> if not (List.mem name env) then
...
let closure_convert_toplevels (tls : Syntax.top_level list) =
  ...
  let closure_convert_toplevel tl =
    ...
    | Syntax.TLAbs (name, params, body) ->
      if name = "main" then
        TLAbs (name, params, convert_expr body params)
      else
        ...
        if List.mem name handle_bodies then
          ...
          TLAbs (lifted_name, ("__env__" :: params), convert_expr body params)
    | Syntax.TLEffSig (name, dcls) ->
      TLEffSig (name, dcls)
    | Syntax.TLObj (name, obj_params, hdls) ->
      let hdls_converted (anno, name, params, body) = (anno, name, params, convert_expr body params) in
